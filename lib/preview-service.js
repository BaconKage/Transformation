...
export async function handlePreviewRequest(req, res) {
  try {
    ...
    const workoutText = customWorkout || picked?.workout || "";
    const dietText = customDiet || picked?.diet || "";
    const planParts = [];
    if (picked) {
      planParts.push(`Selected goal plan: ${picked.name}. Default workout: ${picked.workout}. Default diet: ${picked.diet}.`);
    }
    if (customWorkout || customDiet) {
      planParts.push(`User-provided plan details override defaults when present. Workout: ${customWorkout || "use selected plan default"}. Diet: ${customDiet || "use selected plan default"}.`);
    }
    const planText = planParts.join(" ") || "No plan details provided.";
